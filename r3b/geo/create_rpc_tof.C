...
void create_rpc_tof(const char* geoTag)
{
  ...
  if ( ! pMed0 ) Fatal("Main", "Medium Air not found");

  FairGeoMedium* rpc_al      = geoMedia->getMedium("aluminium");
  if ( ! rpc_al ) Fatal("Main", "FairMedium aluminium not found");
  geoBuild->createMedium(rpc_al);
  TGeoMedium* pMed1 = gGeoMan->GetMedium("aluminium");
  if ( ! pMed1 ) Fatal("Main", "Medium aluminium not found");

  FairGeoMedium* rpc_gas      = geoMedia->getMedium("TimeRpc_gas");
  if ( ! rpc_gas ) Fatal("Main", "FairMedium TimeRpc_gas not found");
  geoBuild->createMedium(rpc_gas);
  TGeoMedium* pMed2 = gGeoMan->GetMedium("TimeRpc_gas");
  if ( ! pMed2 ) Fatal("Main", "Medium TimeRpc_gas not found");
  
  FairGeoMedium* rpc_strip      = geoMedia->getMedium("copper");
  if ( ! rpc_strip ) Fatal("Main", "FairMedium copper not found");
  geoBuild->createMedium(rpc_strip);
  TGeoMedium* pMed3 = gGeoMan->GetMedium("copper");
  if ( ! pMed3 ) Fatal("Main", "Medium copper not found");

  FairGeoMedium* rpc_glass      = geoMedia->getMedium("TimeRpc_glass");
  if ( ! rpc_glass ) Fatal("Main", "FairMedium TimeRpc_glass not found");
  geoBuild->createMedium(rpc_glass);
  TGeoMedium* pMed4 = gGeoMan->GetMedium("TimeRpc_glass");
  if ( ! pMed4 ) Fatal("Main", "Medium TimeRpc_glass not found");

  FairGeoMedium* rpc_pmma      = geoMedia->getMedium("TimeRpc_pmma");
  if ( ! rpc_pmma ) Fatal("Main", "FairMedium TimeRpc_pmma not found");
  geoBuild->createMedium(rpc_pmma);
  TGeoMedium* pMed5 = gGeoMan->GetMedium("TimeRpc_pmma");
  if ( ! pMed5 ) Fatal("Main", "Medium TimeRpc_pmma not found");
  
  FairGeoMedium* rpc_FR4      = geoMedia->getMedium("TimeRpc_FR4");
  if ( ! rpc_FR4 ) Fatal("Main", "FairMedium TimeRpc_FR4 not found");
  geoBuild->createMedium(rpc_FR4);
  TGeoMedium* pMed6 = gGeoMan->GetMedium("TimeRpc_FR4");
  if ( ! pMed6 ) Fatal("Main", "Medium TimeRpc_FR4 not found");

  FairGeoMedium* Vacuum      = geoMedia->getMedium("vacuum");
  if ( ! Vacuum ) Fatal("Main", "FairMedium vacuum not found");
  geoBuild->createMedium(Vacuum);
  TGeoMedium* pMed7 = gGeoMan->GetMedium("vacuum");
  if ( ! pMed7 ) Fatal("Main", "Medium vacuum not found");
  // --------------------------------------------------------------------------
  ...
  //Definition of the mother volume where the rpc voulumes are going to be mounted  
  TGeoVolume *Rpc_module = gGeoManager->MakeBox("R3BRpc", pMed7, xbox/2.0, ybox/2.0, zbox/2.0);
  ...
  TGeoVolume *vol_pmmaLat1 = gGeoManager->MakeBox("pmmalat1", pMed5, det_latxPMMAPos/2.0, det_latyPMMAPos/2.0, det_latzPMMAPos/2.0);
  vol_pmmaLat1->SetLineColor(kYellow);

  TGeoVolume *vol_pmmafront1 = gGeoManager->MakeBox("pmmafront1", pMed5, (det_frontdxPMMAPos)/2.0, det_frontdyPMMAPos/2.0, det_frontdzPMMAPos/2.0);
  vol_pmmafront1->SetLineColor(kYellow);

  TGeoVolume *vol_pmmatop1 = gGeoManager->MakeBox("pmmatop1", pMed5, (det_topdxPMMAPos)/2.0, det_topdyPMMAPos/2.0, det_topdzPMMAPos/2.0);
  vol_pmmatop1->SetLineColor(kYellow);
  

  TGeoVolume *vol_glass = gGeoManager->MakeBox("glass", pMed4, det_xGlassPos/2.0, det_yGlassPos/2.0, det_zGlassPos/2.0);
  ...
  TGeoVolume *Cu_plane = gGeoManager->MakeBox("Cu_plane", pMed3, det_xGlassPos/2.0, det_yGlassPos/2.0, det_zStripPos/2.0);
  Cu_plane->SetLineColor(kBlack);

  TGeoVolume *vol_FrSF6 = gGeoManager->MakeBox("gas1", pMed2, det_xFreonSF6Pos/2.0, det_yFreonSF6Pos/2.0, det_zFreonSF6Pos/2.0);
  vol_FrSF6->SetLineColor(kGreen);

  TGeoVolume *vol_strip = gGeoManager->MakeBox("strip", pMed3, (det_xStripPos)/2.0, (det_yStripPos)/2.0, det_zStripPos/2.0);
...
//add FR4
  ...
  for(j = 1; j <= 40; j++){
    i++;
    Rpc_module->AddNode(vol_strip, i, new TGeoTranslation(0,  -det_yGlassPos/2 + det_yStripPos/2 + (det_yStripPos + 0.1)*(j-1) , (det_zStripPos/2.0) +  det_zFR4Pos +z));
